...
export class ExamFormComponent implements OnInit {
      ...
      const examData = {
        ...
        questions: formValue.questions.map((q: any) => ({
          ...
          answers: q.type === 'MULTIPLE_CHOICE' ? q.answers.map((a: any) => ({
            ...
            text: q.correctAnswer || 'Réponse directe',
            isCorrect: true,
            // Essayer une façon différente d'envoyer les mots-clés
            keywords: q.keywords ? q.keywords.trim() : ''
          }]
        }))
      };
  ...
  onQuestionTypeChange(questionIndex: number): void {
    ...
    if (questionType === 'DIRECT_ANSWER') {
      // Si on passe à une question à réponse directe, on récupère la première réponse correcte des QCM si elle existe
      const answersArray = questionForm.get('answers') as FormArray;
