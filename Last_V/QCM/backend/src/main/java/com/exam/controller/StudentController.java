...
public class StudentController {
    ...
    public ResponseEntity<?> submitExam(@PathVariable Long examId, @RequestBody List<Map<String, Object>> answers) {
        try {
            ...
            Map<String, Object> result = examService.submitExamWithAnswers(exam, student, answers, null);
    ...
    public ResponseEntity<?> submitExam(@RequestBody Map<String, Object> request) {
        try {
            ...
            // Extraire la durée réelle de l'examen si elle est fournie
            Integer durationMinutes = null;
            if (request.containsKey("durationMinutes")) {
                try {
                    durationMinutes = Integer.valueOf(request.get("durationMinutes").toString());
                } catch (NumberFormatException e) {
                    // Si la conversion échoue, on continue sans durée spécifiée
                    System.out.println("Impossible de convertir la durée en entier: " + request.get("durationMinutes"));
                }
            }
            ...
            Map<String, Object> result = examService.submitExamWithAnswers(exam, student, answers, durationMinutes);
    ...
    public ResponseEntity<?> getExamResult(@PathVariable Long examId) {
        try {
            ...
            if (studentExam.getEndTime() != null && studentExam.getStartTime() != null) {
                ...
                resultMap.put("timeSpent", timeSpentMinutes);
                resultMap.put("timeSpentSeconds", timeSpentSeconds);
                resultMap.put("formattedTimeSpent", formattedTimeSpent);
            } else {
                resultMap.put("timeSpent", 0);
                resultMap.put("timeSpentSeconds", 0);
                resultMap.put("formattedTimeSpent", "0 minutes 0 secondes");
            }
            ...
            Set<Long> answeredQuestionIds = studentAnswers.stream()
            ...
            for (Question question : exam.getQuestions()) {
                if (!answeredQuestionIds.contains(question.getId())) {
                    ...
                    unansweredQuestion.put("type", question.getType().toString());
